'use client';

import React, { useEffect, useState } from 'react';
import { Card, Table, Button, Space, Tag, Modal, Form, Input, InputNumber, Switch, message, Popconfirm, Typography, Tooltip, Alert } from 'antd';
import { PlusOutlined, ReloadOutlined, DeleteOutlined, EditOutlined, PlayCircleOutlined, PauseCircleOutlined, CopyOutlined } from '@ant-design/icons';
import { useAppStore } from '@/lib/store';

const bp = process.env.NEXT_PUBLIC_BASE_PATH || '';

const { Title, Text, Paragraph } = Typography;

interface TunnelItem {
  id: number;
  name: string;
  server_id: number;
  server_name: string;
  server_host: string;
  local_port: number;
  remote_host: string;
  remote_port: number;
  auto_start: number;
  status: string;
}

export default function TunnelsPage() {
  const { selectedServer } = useAppStore();
  const [tunnels, setTunnels] = useState<TunnelItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<TunnelItem | null>(null);
  const [form] = Form.useForm();
  const [togglingId, setTogglingId] = useState<number | null>(null);

  const load = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${bp}/api/tunnels`).then(r => r.json());
      setTunnels(res);
    } catch (e) {
      message.error('加载失败');
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => { load(); }, []);

  const handleStart = async (id: number) => {
    setTogglingId(id);
    try {
      const res = await fetch(`${bp}/api/tunnels/${id}/start`, { method: 'POST' }).then(r => r.json());
      if (res.success) message.success('隧道已启动');
      else message.error(res.error || '启动失败');
      load();
    } catch {
      message.error('启动失败');
    } finally {
      setTogglingId(null);
    }
  };

  const handleStop = async (id: number) => {
    setTogglingId(id);
    try {
      await fetch(`${bp}/api/tunnels/${id}/stop`, { method: 'POST' });
      message.success('隧道已停止');
      load();
    } catch {
      message.error('停止失败');
    } finally {
      setTogglingId(null);
    }
  };

  const handleDelete = async (id: number) => {
    try {
      await fetch(`${bp}/api/tunnels/${id}`, { method: 'DELETE' });
      message.success('已删除');
      load();
    } catch {
      message.error('删除失败');
    }
  };

  const handleEdit = (t: TunnelItem) => {
    setEditing(t);
    form.setFieldsValue({
      name: t.name, server_id: t.server_id,
      local_port: t.local_port, remote_host: t.remote_host,
      remote_port: t.remote_port, auto_start: t.auto_start === 1,
    });
    setModalOpen(true);
  };

  const handleAdd = () => {
    setEditing(null);
    form.resetFields();
    form.setFieldsValue({
      remote_host: '127.0.0.1', auto_start: false,
      server_id: selectedServer?.id,
      local_port: 3306, remote_port: 3306,
    });
    setModalOpen(true);
  };

  const handleOk = async () => {
    try {
      const values = await form.validateFields();
      const payload = { ...values, auto_start: values.auto_start ? 1 : 0 };
      if (editing) {
        await fetch(`${bp}/api/tunnels/${editing.id}`, {
          method: 'PUT', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
        message.success('更新成功');
      } else {
        await fetch(`${bp}/api/tunnels`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        });
        message.success('创建成功');
      }
      setModalOpen(false);
      load();
    } catch {}
  };

  const handleCopyCommand = (t: TunnelItem) => {
    const cmd = `ssh -L ${t.local_port}:${t.remote_host}:${t.remote_port} -N root@${t.server_host}`;
    navigator.clipboard.writeText(cmd).then(() => {
      message.success('已复制到剪贴板');
    }).catch(() => {
      Modal.info({ title: '本地 SSH 命令', content: <Input value={cmd} readOnly style={{ marginTop: 8 }} /> });
    });
  };

  const columns = [
    {
      title: '名称', dataIndex: 'name', key: 'name',
      render: (t: string) => <strong>{t}</strong>,
    },
    {
      title: '服务器', key: 'server',
      render: (_: unknown, r: TunnelItem) => (
        <span>{r.server_name} <Text type="secondary" style={{ fontSize: 12 }}>({r.server_host})</Text></span>
      ),
    },
    {
      title: '端口转发', key: 'forward',
      render: (_: unknown, r: TunnelItem) => (
        <Space size={4}>
          <Tag color="blue">本地:{r.local_port}</Tag>
          {'→'}
          <Tag color="cyan">{r.remote_host}:{r.remote_port}</Tag>
        </Space>
      ),
    },
    {
      title: '自启', dataIndex: 'auto_start', key: 'auto_start', width: 60,
      render: (v: number) => v ? <Tag color="green">是</Tag> : <Tag>否</Tag>,
    },
    {
      title: '状态', dataIndex: 'status', key: 'status', width: 90,
      render: (s: string) => s === 'running'
        ? <Tag color="success">● 运行中</Tag>
        : <Tag color="default">○ 已停止</Tag>,
    },
    {
      title: '操作', key: 'actions', width: 340,
      render: (_: unknown, r: TunnelItem) => (
        <Space size={4}>
          {r.status === 'running' ? (
            <Button size="small" icon={<PauseCircleOutlined />} loading={togglingId === r.id} onClick={() => handleStop(r.id)}>停止</Button>
          ) : (
            <Button size="small" type="primary" icon={<PlayCircleOutlined />} loading={togglingId === r.id} onClick={() => handleStart(r.id)}>启动</Button>
          )}
          <Tooltip title="复制本地 SSH 命令">
            <Button size="small" icon={<CopyOutlined />} onClick={() => handleCopyCommand(r)}>命令</Button>
          </Tooltip>
          <Button size="small" type="text" icon={<EditOutlined />} onClick={() => handleEdit(r)} />
          <Popconfirm title="确定删除？" onConfirm={() => handleDelete(r.id)}>
            <Button size="small" type="text" danger icon={<DeleteOutlined />} />
          </Popconfirm>
        </Space>
      ),
    },
  ];

  return (
    <Card
      title={<Title level={4} style={{ margin: 0 }}>SSH 隧道</Title>}
      extra={
        <Space>
          <Button icon={<ReloadOutlined />} onClick={load}>刷新</Button>
          <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>添加隧道</Button>
        </Space>
      }
    >
      <Alert
        type="info" showIcon style={{ marginBottom: 16 }}
        message="使用方式"
        description={
          <div>
            <Paragraph style={{ margin: '4px 0' }}>
              <Text strong>复制命令</Text>：在本地终端执行，端口开在你本地电脑。
            </Paragraph>
            <Paragraph style={{ margin: '4px 0' }}>
              <Text strong>启动</Text>：端口开在面板服务器上，需中转访问。
            </Paragraph>
          </div>
        }
      />
      <Table
        columns={columns}
        dataSource={tunnels}
        rowKey="id"
        loading={loading}
        pagination={false}
        size="middle"
      />

      <Modal
        title={editing ? '编辑隧道' : '添加隧道'}
        open={modalOpen} onOk={handleOk} onCancel={() => setModalOpen(false)}
        width={480} okText={editing ? '保存' : '创建'} cancelText="取消"
      >
        <Form form={form} layout="vertical">
          <Form.Item name="name" label="名称" rules={[{ required: true, message: '请输入名称' }]}>
            <Input placeholder="如：MySQL 隧道" />
          </Form.Item>
          <Form.Item name="server_id" label="服务器" rules={[{ required: true, message: '请选择服务器' }]}>
            <InputNumber style={{ display: 'none' }} />
            <Text>{editing?.server_name || selectedServer?.name || '请在服务器页面选择'}</Text>
          </Form.Item>
          <Space.Compact style={{ width: '100%' }}>
            <Form.Item name="local_port" label="本地端口" rules={[{ required: true }]} style={{ width: '50%', paddingRight: 8 }}>
              <InputNumber min={1} max={65535} style={{ width: '100%' }} placeholder="3306" />
            </Form.Item>
            <Form.Item name="remote_port" label="远程端口" rules={[{ required: true }]} style={{ width: '50%' }}>
              <InputNumber min={1} max={65535} style={{ width: '100%' }} placeholder="3306" />
            </Form.Item>
          </Space.Compact>
          <Form.Item name="remote_host" label="远程主机" rules={[{ required: true }]}>
            <Input placeholder="127.0.0.1" />
          </Form.Item>
          <Form.Item name="auto_start" label="自动启动（服务器端隧道）" valuePropName="checked">
            <Switch />
          </Form.Item>
        </Form>
      </Modal>
    </Card>
  );
}
